...
class AutoStepApp(tk.Tk):
    def __init__(self):
        ...
        # Main frame now uses a canvas for scrolling
    ...
    def create_menu(self):
        ...
        helpmenu.add_command(
            label="Shortcuts", command=self.show_shortcuts)
    ...
    def show_info_repeat(self):
        messagebox.showinfo(
            "Repeat Count Info", "Number of times to repeat the playback of the selected recording. Must be a positive integer (e.g., 1, 2, 3).")

    def show_info_delay(self):
        messagebox.showinfo(
            "Delay Info", "Delay between consecutive playbacks in seconds. Use 0 for no delay. Must be a non-negative number (e.g., 0, 1.5, 2).")

    def playback_flow(self):
        self.clear_frame()
        self.selected_playback_file = None  # Reset selection

        # Back button with arrow icon at top left
        back_btn = ttk.Button(self.main_frame, text="←",
                              width=3, command=self.create_initial_screen)
        ...
        recordings = [f for f in os.listdir(
            RECORDINGS_FOLDER) if f.endswith(".json") and os.path.exists(os.path.join(RECORDINGS_FOLDER, f))]
        self.recordings_listbox.delete(0, tk.END)  # Clear existing items
        ...
        # Rowed inputs
        ttk.Label(self.playback_controls_frame, text="Repeat").grid(
        ...
        ttk.Button(self.playback_controls_frame, text="?", width=2,
                   command=self.show_info_repeat).grid(row=0, column=2, padx=5)

        ttk.Label(self.playback_controls_frame, text="Delay").grid(
        ...
        ttk.Button(self.playback_controls_frame, text="?", width=2,
                   command=self.show_info_delay).grid(row=0, column=5, padx=5)
        ...
        self.play_log = tk.Text(
            self.main_frame, height=8, state="disabled")
    ...
    def select_recording_in_list(self, filename):
        ...
        # If filename is an absolute path, use as-is; else join with RECORDINGS_FOLDER
        if os.path.isabs(filename):
            full_path = filename
        else:
            full_path = os.path.join(RECORDINGS_FOLDER, filename)
        self.selected_playback_file = full_path
        if os.path.exists(self.selected_playback_file):
            self.play_btn['state'] = 'normal'
            self.log_play(f"Selected file: {os.path.basename(self.selected_playback_file)}")
        else:
            self.play_btn['state'] = 'disabled'
            self.selected_playback_file = None
            self.log_play(f"Error: File {filename} does not exist.")
    ...
    def select_playback_file(self):
        ...
        if f:
            ...
            self.selected_playback_file = f  # Use full path directly
            self.select_recording_in_list(f)  # Pass full path
            self.log_play(f"Manually selected file: {basename}")
        else:
            self.play_btn['state'] = 'disabled'
            self.selected_playback_file = None
            self.log_play("No file selected manually.")
    ...
    def show_countdown(self, seconds, callback, cancelable=False):
        ...
        def countdown(n):
            if self.countdown_cancelled:
                # Restore the appropriate screen based on the callback context
                if callback.__name__ == "start_recording":
                    self.create_initial_screen()
                elif callback.__name__ == "playback_start":
                    self.playback_flow()  # Restore playback screen
                return
            if n > 0:
    ...
    def start_recording(self):
        ...
        self.log_text = tk.Text(
            self.main_frame, height=10, state="disabled")
    ...
    def save_recording(self):
        ...
        try:
            ...
            self.log(
                f"Saved recording to: {os.path.basename(self.filename)}")
        except Exception as e:
            self.log(f"Error saving file: {e}")
            messagebox.showerror(
                "Error", f"Could not save recording:\n{e}")
    ...
    def recording_selected(self, event=None):
        ...
        self.selected_playback_file = os.path.join(
            RECORDINGS_FOLDER, filename)
        if os.path.exists(self.selected_playback_file):
            self.play_btn['state'] = 'normal'
            self.log_play(f"Selected file: {filename}")
        else:
            self.play_btn['state'] = 'disabled'
            self.selected_playback_file = None
            self.log_play(f"Error: File {filename} does not exist.")
            messagebox.showerror(
                "Error", f"Selected file {filename} does not exist.")

    def play_selected_recording(self):
        if not self.selected_playback_file or not os.path.exists(self.selected_playback_file):
            messagebox.showwarning(
                "No Selection", "Please select a valid recording file first.")
            self.play_btn['state'] = 'disabled'
            return
    ...
    def playback_start(self, repeat_count, repeat_delay):
        # Only clear the log if the widget still exists
        try:
            if self.play_log.winfo_exists():
                self.play_log.config(state="normal")
                self.play_log.delete("1.0", tk.END)
                self.play_log.config(state="disabled")
        except Exception:
            pass  # Widget does not exist, skip clearing
    ...
    def playback(self, repeat_count, repeat_delay):
        if not self.selected_playback_file or not os.path.exists(self.selected_playback_file):
            self.log_play("Error: No valid recording file selected.")
            messagebox.showerror("Error", "No valid recording file selected.")
            return
